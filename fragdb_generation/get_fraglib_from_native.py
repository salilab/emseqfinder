...
def get_SSE_fragments(stride_file):
    '''
    get all the SSE fragments info for the native structure from its stride file

    '''
    current_chain = None
    stride_dict = {}
    frag_list = []
    # 1. collect all helix and strand info as a dictionary, where the information is stored based on chain id as well
    with open(stride_file, 'r') as infile:
        for line in infile:
            if line.startswith('ASG '):
                ...
                if current_chain == None or chain_id != current_chain:
    ...
    for chainid in stride_dict:
        ...
        for resid in stride_dict[chainid]:
            ...
            if previous_ss == None:
                ...
                previous_res = resid
	    # only contiguous residues with same SS type should be part of a fragment
            if current_ss == previous_ss and resid - previous_res < 2:
                ...
                new_key = chainid + '_' + current_frag[0][1] + '_' + str(first_res) + '_' + str(last_res) + '_' + str(length)
...
def get_fragment_coords(pdbname, frag_info, path_to_store_parts):
    ...
    also save the pdbs of the helical fragments and check if for strands there could be multiple strands possibilities
    ...
    for frag in frag_info:
        ...
        if int(length) >=3:
            if SSEtype == 'H':
                # write a pdb now
                h_frag = IMP.atom.Selection(native_h, chain_id=chainid, residue_indexes=range(int(firstres), int(lastres)+1), atom_types=[IMP.atom.AtomType(n) for n in selected_atom_types])
                pdb_name = '_'.join(['h', length, chainid , firstres, lastres]) + '.pdb'
                IMP.atom.write_pdb(h_frag, outdir+ '/' + pdb_name)
                all_pdbs.append(outdir+ '/' + pdb_name)
            elif SSEtype == 'E':
                # collect the coordinates first
                s_frag = IMP.atom.Selection(native_h, chain_id=chainid, residue_indexes=range(int(firstres), int(lastres)+1), atom_types=[IMP.atom.AtomType(n) for n in selected_atom_types])
                pdb_name = '_'.join(['s', length, chainid , firstres, lastres]) + '.pdb'
                IMP.atom.write_pdb(s_frag, outdir+ '/' + pdb_name)
                all_pdbs.append(outdir+ '/' + pdb_name)
            else:
                print('Something is wrong, should only process strands and helix')
    return all_pdbs   
...
def per_chain_segment_coords(pdbname, segment_info, path_to_store_parts):
    ...
    for chains in segment_info.keys():
        ...
        # write a pdb now
        h_frag = IMP.atom.Selection(native_h, chain_id=chains, residue_indexes=all_residueids, atom_types=[IMP.atom.AtomType(n) for n in selected_atom_types])
        pdb_name = '_'.join(['all', str(len(all_residueids)), chains , str(all_residueids[0]), str(all_residueids[-1])]) + '.pdb'
        IMP.atom.write_pdb(h_frag, outdir+ '/' + pdb_name)
        all_pdbs.append(outdir+ '/' + pdb_name)
    return all_pdbs
...
parser = argparse.ArgumentParser(description='Generate a library of parts from native structure, using the information from STRIDE')
parser.add_argument('ref_pdb', type=str,
                    help='Provide the absolute path of the native pdb (we assumed that as reference PDB)')
...
if not args.perChain:
    all_new_pdbs = get_fragment_coords(args.ref_pdb, all_frags, args.path_to_store_parts)
else:
    all_new_pdbs = per_chain_segment_coords(args.ref_pdb, stride_dict, args.path_to_store_parts)
#for pdbname in all_new_pdbs:
    #mutate_frag_to_ala(pdbname)
