...
import time
# This script contains all important scripts for preparing input file
# and running the ML model

residues = ["GLY", "ALA", "THR", "HIS", "SER", "ASP", "ASN", "GLU", "GLN",
...
def import_voxel_file_pandas(fname, other_columns, n_voxels, read_rows='all'):
    ...
    if read_rows == 'all':
    ...
    elif type(read_rows) is int or type(read_rows) is float:
        database = pd.read_csv(fname, sep=" ", names=columns, nrows=read_rows,
                               na_values=["None"], dtype=dtypes)
    else:
        ...
        database = pd.read_csv(fname, sep=" ", names=columns,
                               na_values=["None"], dtype=dtypes)
    t0 = time.time()
    print("Extracted "+str(len(database))+" lines in "+str(time.time()-t0),
          flush=True)
...
def import_voxel_file_pickle(fname, other_columns, n_voxels):
    ...
    print("Database size (MB):", str(sys.getsizeof(database)/1024/1024),
          len(database))
...
def import_voxel_file(fname, other_columns, n_voxels):
    ...
    for line in open(fname, "r"):
        k += 1
        if k % 100000 == 0:
            print("-- Line "+str(k)+" entries: "+str(linenum)+" in "
                  + str(time.time()-t0), flush=True)
        try:
            int(line[0])
        except:  # noqa: E722
        ...
        for i in range(len(fields)):
            ...
            data.append(dlist)
        linenum += 1
    print("Extracted "+str(len(data))+" lines in "+str(time.time()-t0),
          flush=True)
    t0 = time.time()
    database = pd.DataFrame(data, columns=columns)
...
def split_image_and_other_features(df, image_prefix="v",
                                   other_columns=["resolution"]):
    # Split a dataframe into two dataframes: one with image information,
    # one with other features
    # other_columns :: column labels to extract and return in
    # other_features - these should be ones used for learning
...
class ResidueVoxelDataset:
    def __init__(self, infile, classes=residues, target=None,
        ...
        self.data_df = import_voxel_file_pickle(
            self.infile, other_columns=other_columns,
...
#            print(bc)
        ...
        if add_to_training_features:
            self.train_features = [y for x in [self.train_features, ss_cats]
                                   for y in x]
#        print(self.data_df[c])
    ...
    def split_features_and_reshape(self, df):
        ...
        ximages, xother = split_image_and_other_features(
            df, other_columns=self.train_features)
    ...
    def get_train_test_val_sets(self, train=0.75, test=0.15, val=0.1):
        df_train, df_test, df_val = split_df_random(
            self.data_df, train, test, val)
