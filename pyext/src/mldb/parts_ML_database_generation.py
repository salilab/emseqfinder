...
for expmappdb in expmappdb_list:
    ...
    os.system('python3 ' + './normalize_map_for_parts_fiiting.py' + ' '
              + emdb + ' --thresh ' + threshold)
    # now extract side chain densities
    datbase_path = emdb + '_ML_side.dat'
    os.system('python3 ' + './get_database_for_one_emdb_using_parts.py' + ' '
              + emdb + ' ' + datbase_path + ' ' + resolution)
