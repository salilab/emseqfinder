...
residues = ['ALA', 'ARG', 'ASN', 'ASP', 'CYS', 'GLN', 'GLU', 'GLY', 'HIS',
            ...
            'TYR', 'VAL']
features = ["EMDB", "resolution", "chain", "resid", "resname", "ss", "se_ccc",
            "res_ccc"]
...
h_residue_propensities = {
    'ALA': 0.09743660383807928, 'ARG': 0.052206653248949665,
    'ASN': 0.033516232631581634, 'ASP': 0.03697014685512692,
    'CYS': 0.014795201253000493, 'GLN': 0.04344432285169852,
    'GLU': 0.06778753070641984, 'GLY': 0.03754920042140815,
    'HIS': 0.01862409730140631, 'ILE': 0.07440455692198593,
    'LEU': 0.14053655562613038, 'LYS': 0.05310967070473625,
    'MET': 0.030689839012904986, 'PHE': 0.05122200709659481,
    'PRO': 0.017251714840352126, 'SER': 0.05328313168494384,
    'THR': 0.04712016509403881, 'TRP': 0.016744086383568144,
    'TYR': 0.038248146135774035, 'VAL': 0.07506013739129991}

s_residue_propensities = {
    'ALA': 0.06003398703023674, 'ARG': 0.044378467067739666,
    'ASN': 0.03278576451285257, 'ASP': 0.03541292288460036,
    'CYS': 0.018204547230252364, 'GLN': 0.03322525197281038,
    'GLU': 0.04347995937182592, 'GLY': 0.04848035002734589,
    'HIS': 0.017638096726306743, 'ILE': 0.09542737713883898,
    'LEU': 0.10406086413001016, 'LYS': 0.04244472224392531,
    'MET': 0.02161301664192515, 'PHE': 0.05539495273068208,
    'PRO': 0.019483944058129542, 'SER': 0.06141104773810454,
    'THR': 0.0748593640128135, 'TRP': 0.015216032502539261,
...
def process_input_database(db, reslow=0.0, reshigh=10.0, normalize=True,
                           threshold=True, nvoxels=2744):
...
def import_voxel_file(fname):
    f = open(fname, "r")
    columns = ["EMDB", "resolution", "chain", "resid", "resname", "ss",
               "se_ccc", "res_ccc"]
    data = []
...
def score_prior_over_against_sequences(sedf, sequences, prior,
                                       unavailable=None):
    se_len = len(sedf)
    scores = {}
...
def score_sesf_over_against_sequences(sedf, sequences, count=False,
                                      unavailable=None, log=True):
    se_len = len(sedf)
    scores = {}
...
def get_se_ss(df_se):
    ...
    if len(ss) > 1:
        print("Multiple secondary structures in this element!  That's bad!!",
              df_se["emdb"].values[0])
        # print(df_se["resid"])
    return ss[0]
...
def score_sequence(sedf, seq, log=True):
    if len(seq) != len(sedf):
        raise Exception("Input sequence and structure element must be "
                        "the same size")
    score = 0
    for i in range(len(seq)):
        if not log:
    ...
    for c in chains:
        ...
        for i in range(1, len(sorted_resids)):
            ...
            if ridx - rid_1 > 1 or this_ss != ss:
                if rid_1-rid_0 > 3:
                    seid = (ss+"_"+c+"_"+str(int(rid_0))+"_"
                            + str(int(rid_1-rid_0)+1))

                    idx = np.where((df_chain['resid'] >= rid_0)
                                   & (df_chain['resid'] <= rid_1))
